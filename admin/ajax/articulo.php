 ...
 /*  case 'listar':
        ...
        while ($reg = $rspta->fetch_object()) {
            $data[] = array(
                ...
                "0" => ($reg->estado)? '<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->idarticulo .')"><i class="fa fa-pencil"></i></button>'.' 
                '.'<button class="btn btn-danger btn-xs" onclick="eliminar('.$reg->idarticulo.')"><i class="fa fa-trash"></i></button>'.' 
                '.'<button class="btn btn-danger btn-xs" onclick="desactivar('.$reg->idarticulo .')"><i class="fa fa-close">
                </i></button>':'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->idarticulo .')"><i class="fa fa-pencil"></i></button>'.' '.'<button class="btn btn-info btn-xs" onclick="mostrar_clave('.$reg->idarticulo.')">
                <i class="fa fa-key"></i></button>'.' '.'<button class="btn btn-primary btn-xs" onclick="activar('.$reg->idarticulo .')"><i class="fa fa-check"></i></button>',
                "1" => $reg->nombre,
                "2" => $reg->codigo,
                "3" => $reg->descripcion,
                "4" => $reg->marca,
            ...
            while ($reg = $rspta->fetch_object()) {
                $data[] = array(
                    "0" => ($reg->estado) 
                        ...
                        : '<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->idarticulo.')"><i class="fa fa-pencil"></i></button> 
                           <button class="btn btn-info btn-xs" onclick="mostrar_clave('.$reg->idarticulo.')"><i class="fa fa-key"></i></button> 
                    ...
                    "8" => $reg->ram,
                    "9" => $reg->rom,
                    "10" => $reg->categoria,  // Mostrar el nombre de la categoría
                    "11" => $reg->fechacreada
                );
            }
